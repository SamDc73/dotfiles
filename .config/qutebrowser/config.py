...
# config.load_autoconfig()c.aliases = {"q": "quit", "w": "session-save", "wq": "quit --save"}

# Theme
config.source("themes/monokai.py")
...
# Use neovim in kitty as an editor
c.editor.command = [
    "kitty",
    "-t",
    "Qutebrowser Editor",
...
config.bind(
    ";ig",
    "hint images run open https://www.google.com/searchbyimage?&image_url={hint-url}",
)
...
# Remove finished downloads from the download bar after 3 minutes
c.downloads.remove_finished = 200000
...
# When to show the tab barabs.
c.tabs.show = "always"
c.tabs.show = "multiple"  # To Hide the bar if only one tab is open
...
config.set("content.cookies.accept", "all", "https://www.reddit.com")
